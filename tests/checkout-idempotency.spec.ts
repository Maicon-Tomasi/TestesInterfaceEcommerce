...
test.describe.serial('Checkout Idempotency - Concorrência via API', () => {
  ...
  test('Cenário 1: Múltiplas requisições paralelas com a mesma Idempotency-Key', async ({ page, request }) => {
    ...
    // 2. Setup de Usuário
    const testUser = { name: 'User Idemp', email: `uidemp_${Date.now()}@test.com`, password: 'Password123!', document: generateValidCPF(), phone: '11999999999' };
    ...
    try {
      ...
      // Se não redirecionou, DEVE exibir alguma mensagem de erro na tela (409, 500 ou Network Error)
      // Como a mensagem exata pode variar dependendo do momento do bloqueio ou de timeouts da API no "cold start",
      // vamos ser mais tolerantes e buscar pelo container de erro vermelho da interface.
      const redErrorDiv = page.locator('.text-red-600').first();
      
      try {
        await expect(redErrorDiv).toBeVisible({ timeout: 5000 });
        const actualText = await redErrorDiv.textContent();
        console.log(`[SUCESSO] Idempotência bloqueou corretamente. Mensagem exibida: "${actualText}"`);
      } catch (innerError) {
        console.error(`[FALHA] Nenhuma mensagem de erro vermelha foi encontrada na tela. A tela atual é: ${page.url()}`);
        throw innerError;
      }
    }
  });
